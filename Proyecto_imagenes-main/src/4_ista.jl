...
# --- Funciones Auxiliares (Necesarias para que FISTA funcione) ---
...
# Definición de operadores de Wavelet
W_op(x) = haar2d_forward(x)
W_adj(c) = haar2d_inverse(c)

# Definición del gradiente (Necesario para el paso 1 de FISTA)
...
# --- Tu Nueva Función FISTA ---

function fista_l1(y, mask; λ=0.01, maxiter::Int=100, α::Float64=0.8, x0=nothing, verbose::Bool=true)
    # Inicialización
    x = x0 === nothing ? real(F_adj(y)) : copy(x0)
    
    # Variables extra para FISTA (momento)
    x_old = copy(x)
    z = copy(x) # Punto de extrapolación
    t = 1.0
    
    for it in 1:maxiter
        # 1. Gradiente sobre el punto extrapolado 'z', no sobre 'x'
        g = grad_f(z, mask, y)
        z_tmp = z .- α .* g
        
        # 2. Proximal (Soft Thresholding)
        c = W_op(z_tmp)
        c_thr = soft_threshold(c, α * λ)
        x_new = W_adj(c_thr)
        
        # 3. Actualización de momento (Paso FISTA)
        t_new = (1.0 + sqrt(1.0 + 4.0 * t^2)) / 2.0
        z = x_new + ((t - 1.0) / t_new) * (x_new - x_old)
        
        # Calcular diferencia para convergencia
        diff = norm(x_new - x_old) / max(norm(x_new), eps())
        ...
        if verbose && (it % 10 == 0)
            @info "FISTA iter $it, diff = $diff"
        end
        if diff < 1e-5 # Criterio un poco más estricto
